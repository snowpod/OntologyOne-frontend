...
const FeedbackInput = forwardRef(({ onFeedback, onExpand }, ref) => {
  ...
  const handleSliderChange = (e) => {
    ...
    // Update prefilled text only if user hasn’t typed yet
    if (!hasUserTyped && comment === prefilled) {
      const newPrefill = `${placeholderMap[newRating - 1]} because `;
      setComment(newPrefill);
  ...
  const handleTextareaFocus = () => {
    if (!comment.trim()) {
      const fill = `${placeholderMap[rating - 1]} because `;
      setPrefilled(fill);
      ...
      requestAnimationFrame(() => {
        ...
        if (textarea) {
          textarea.selectionStart = textarea.selectionEnd = fill.length;
          textarea.focus();
        }
      });
    }
  ...
  const handleKeyDown = (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      ...
      if (comment.trim()) {
        ...
        const mismatch =
          initialRatingAtFirstInput !== null &&
          initialRatingAtFirstInput !== rating;

        onFeedback?.({ comment, rating });
        setReply(generateReply(sentiment, mismatch));
        setIsSubmitted(true);
      }
    }
  };
        ...
        <div className={styles.sliderWrapper}>
          ...
          <div className={styles.textareaContainer}>
            <textarea
              ...
              placeholder={`${placeholderMap[rating - 1]} because... Press Enter to send`}
              disabled={isSubmitted}
              className={`${styles.textarea} ${prefilled && comment === prefilled ? styles.prefilled : "text-black"}`}
            />
